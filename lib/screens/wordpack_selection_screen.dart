...
class _WordpackSelectionScreenState extends State<WordpackSelectionScreen> {
    ...
    _rewardedAd!.show(
      onUserEarnedReward: (AdWithoutView ad, RewardItem reward) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Ad Completed! Packs temporarily unlocked.', style: TextStyle(fontWeight: FontWeight.bold))),
          );
        }
      }
    );
  ...
  Future<void> _processPremiumPurchase() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _isProcessingTransaction = false);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Welcome to Premium!', style: TextStyle(fontWeight: FontWeight.bold)), backgroundColor: Colors.amber),
        );
      }
    } on PlatformException catch (e) {
      var errorCode = PurchasesErrorHelper.getErrorCode(e);
        ...
        children: [
          Column(
            children: [
              ...
              // THE NEW PERSISTENT PREMIUM BANNER
  ...
  Widget _buildPremiumBanner(ThemeData theme) {
    return Container(
      ...
      padding: EdgeInsets.only(
        ...
        bottom: MediaQuery.of(context).padding.bottom + 24 // Safe area padding
      ),
      decoration: BoxDecoration(
        color: const Color(0xFF1A1A1A),
        border: Border(top: BorderSide(color: Colors.amber.withAlpha(100), width: 2)),
        boxShadow: [
          BoxShadow(color: Colors.amber.withAlpha(10), blurRadius: 20, offset: const Offset(0, -5))
        ]
      ),
      child: Column(
        ...
        children: [
          const Row(
            ...
            children: [
              ...
              SizedBox(width: 12),
              Text("LEXICON PREMIUM", style: TextStyle(color: Colors.amber, fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: 2)),
            ],
          ),
          ...
          ElevatedButton(
            ...
            child: const Text("BUY FOR \$7.99", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
          ),
        ],
      ),
    );
  ...
  Widget _premiumBullet(String text) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(Icons.check_circle_outline, color: Colors.amber.withAlpha(200), size: 18),
            ...
            children: [
              ...
              if (isLocked) ...[
                ...
                if (pack.requiresPremium)
                  _premiumBadge() // Now it just shows the badge, the Buy button is at the bottom
                else
                  Row(
                    children: [
                      Expanded(child: _actionBtn("WATCH AD", Icons.play_circle_outline, Colors.white10, _processAdReward)),
                      const SizedBox(width: 8),
                      Expanded(child: _actionBtn("BUY \$0.99", Icons.shopping_bag_outlined, theme.colorScheme.primary.withAlpha(40), () => _processPurchase(pack.id))),
                    ],
                  ),
